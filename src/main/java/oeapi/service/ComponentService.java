...
//        return componentOffering;
    ...
    public boolean delete(String id) {
        ...
        Optional<Component> componentExisting = this.getById(id);
        if (!componentExisting.isPresent()) {
            throw new oeapiException(HttpStatus.BAD_REQUEST, "Component not found", "Component [" + id + "] not found");

        }
        Component component = componentExisting.get();
        logger.debug("*>-Component Service: Deleting Component with ID:"+id); 
               
        // Clear all rest relatons
        component = this.clearRelations(component);
        super.delete(id);
        return true;
    }    
        
    public Component clearRelations(Component component) {

        List<ModeOfDelivery> modeOfDelivery = component.getModeOfDelivery();
