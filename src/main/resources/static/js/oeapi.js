...
async function loadFullCourseData(univShortName, courseID) {
    ...
    try {
        ...
        if (offeringsJSON.items[1])  // has two offerings
        {
            ...
            mainOffering = offeringsJSON.items[0]; // Initial guess            
            virtualOffering = offeringsJSON.items[1];
            ...
            {
                virtualOffering = offeringsJSON.items[0];
                mainOffering = offeringsJSON.items[1];
            }
        } else
        {
...
<div class="row">
  ...
  <div class="course-actions">
    ...
    <div class="delete-icon" id="deleteIcon" onclick="deleteCourse(theCourse,true)" style="cursor: pointer;">
        Delete course <i class="fas fa-trash fa-1x"></i>
    </div>
  </div>
